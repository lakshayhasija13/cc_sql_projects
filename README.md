# cc_sql_projects

## Projects Overview

| Project Name (Directory) | Display Name | Difficulty Rating |
|---|---|---|
| customer_segmentation | Customer Segmentation | ⭐ Beginner |
| exchange | Exchange | ⭐⭐ Beginner-Intermediate |
| fraudulent_transactions | Fraudulent Transactions | ⭐ Beginner |
| hacker_news | Hacker News | ⭐⭐⭐ Intermediate |
| lyft_trips | Lyft Trips | ⭐⭐⭐ Intermediate |
| multiple_table_reddit | Multiple Table Reddit | ⭐⭐⭐⭐ Advanced |
| museum_art | Museum Art | ⭐⭐ Beginner-Intermediate |
| new_york_restaurant | New York Restaurant | ⭐⭐ Beginner-Intermediate |
| subway_ad | Subway Ad | ⭐ Beginner |
| trends_in_startups | Trends in Startups | ⭐⭐⭐ Intermediate |
| welp | Welp | ⭐⭐⭐ Intermediate |

### Rating Scale
| Rating | Level | Concepts Covered |
|---|---|---|
| ⭐ | Beginner | SELECT, WHERE, LIKE, IS NULL, AND/OR |
| ⭐⭐ | Beginner-Intermediate | Aggregate functions (SUM, COUNT, AVG, MAX, MIN), GROUP BY, CASE WHEN |
| ⭐⭐⭐ | Intermediate | JOINs, UNION, HAVING, date functions, complex filtering |
| ⭐⭐⭐⭐ | Advanced | CTEs (WITH), multiple JOINs, UNION, complex subqueries |
